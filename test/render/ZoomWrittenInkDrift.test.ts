 ...
 it(`padding=${padding} scale=${scale} measured=${measured} pinch=${pinch} cancel=${cancel}`,async()=>{
  ...
  try{
   ...
   try{
    ...
    const compare=(actual:any)=>{expect(actual.blue.count).toBeGreaterThan(0);for(const key of ["minX","minY","maxX","maxY"] as const)expect.soft(Math.abs(actual.blue[key]-correct.blue[key])).toBeLessThan(.75);};
    compare(raster);
    ...
    try{expect(cold.initial.loads).toBe(1);expect(cold.initial.strokes[0]).toEqual(initial.strokes[0]);expect.soft(cold.initial.strokes.at(-1).points[0].y).toBeCloseTo(200,1);
     const reopened=await cold.page.evaluate(()=>(window as any).zoomDrift.raster());stableOld(reopened);compare(reopened);
